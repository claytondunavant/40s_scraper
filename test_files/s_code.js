...
Manual Update Version: H.24.1.m9
Manual Update Date: 2013-Jun-12
File Size: 47231 bytes*/
/************************ ADDITIONAL FEATURES ************************
     Universal Tag
     Plugins
*/
...
function s_doPlugins(s) {
	/* Add name attributes links */
		if(SXM && SXM.analytics){			
			if(typeof s.linkType === "undefined" || s.linkType === ""){  // not from s.tl()
				s.eventTarget = s.p_gh();
				if(s.eventTarget && s.eventTarget.nodeName === "A"){
					$target = $(s.eventTarget);
					if(!$target.attr("name")){
						var attrs = SXM.analytics.getClosestTrackingAttributes($target);
						if (attrs.lid === "") attrs.lid = $target.escapedText();
						if (attrs.lpos === "" && tpn) attrs.lpos = tpn;
						var nameval = "&lid=" + attrs.lid + "&lpos=" + attrs.lpos;
	...
	/* JSA added 1/29/2013 ITQ 88887 */
		s.tnt=s.trackTNT();
    
    // Sid - added platform to c50/v50 - SXMCOM-1340
        s.eVar50 = s.getUserPlatform();
        s.prop50 = s.getUserPlatform(); 
		
}

s.getUserPlatform = function(){ 
        var platformString = "";
        var cookies = $(document.cookie.split(";")).map(function(){
            return this.trim();
        });
        for(i in cookies){ 
            if(typeof cookies[i] == "string" && cookies[i].indexOf("sxm_platform=") != -1){
                platformString = cookies[i].split('=')[1];
                break;
            }
        }
